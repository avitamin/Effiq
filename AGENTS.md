...
- The repo now contains a Tauri v2 desktop app for the Effiq review runner plus repository documentation.
- `README.md` owns the project overview and current product intent.
- `docs/tauri-review-runner.md` owns durable app usage, validation, and security notes.
...
- In user-visible Markdown, link semantic Jira issue mentions as `[AG-123]({baseUrl}/browse/AG-123)` using local `.local/effiq.settings.json` first and an already available trusted MCP `browse_url` only as fallback; keep code, commands, JQL, and raw snippets unchanged.
